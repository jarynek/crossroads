...
export class SearchComponent implements OnInit, OnDestroy {
    ...
    @Input() collapseInit: boolean;
    @Output() collapseInitUpdate = new EventEmitter();
    ...
    private filterNavigation(value: string): void {
        ...
        this._tree.forEach((tree: InterfaceTree) => {
            const result = tree.items.filter((item: InterfaceCrossroads) => item.hidden === false);
            tree.open = !!result.length;
            tree.count = result.length;
        });
    }
    ...
    public collapseNavigateInit(): void {
        let init = false;
        this._tree.forEach((tree: InterfaceTree) => {
            tree.items.map((item: InterfaceCrossroads) => {
                if (!item.hidden) {
                    init = true;
                }
            });
        });
        this.collapseInitUpdate.next(init);
    }
}
